/*
 * LeetCode Problem 3831: Find X Value of Array I
 * Problem Number: 3831
 * Difficulty: Medium
 ...
 *         for (int i = 0; i < n; i++) {
 *             int m = nums[i] % k;
 *             for (int v = 0; v < k; v++) tmp[v] = 0;
 *             for (int v = 0; v < k; v++) {
 *                 if (cnt[v] !=0)tmp[(v * m) % k] += cnt[v];
 *             }
 *             tmp[m] += 1;
 *             for (int v = 0; v < k; v++) cnt[v] = tmp[v];
 *             for (int v = 0; v < k;v++) res[v] += cnt[v];
 *         }
 *         return res;
 *     }
 * }
