...
class Solution {
    ...
    private int quickSelect(int[] nums, int left, int right, int target) {
        ...
        if (target == pivotIndex) return nums[target];
        else if (target < pivotIndex) return quickSelect(nums, left, pivotIndex - 1, target);
        else return quickSelect(nums, pivotIndex + 1, right, target);
    }
