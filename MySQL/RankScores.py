...
def rank_scores(scores: pd.DataFrame) -> pd.DataFrame:
    """
    Rank scores using dense ranking (competition ranking).
    
    Args:
        scores: DataFrame with columns 'id' and 'score'
        
    Returns:
        DataFrame with columns 'score' and 'rank'
    """
    # Sort by score descending
    result = scores[['score']].copy()
    result = result.sort_values('score', ascending=False)
    
    # Calculate dense rank using pandas
